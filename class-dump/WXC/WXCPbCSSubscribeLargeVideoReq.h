//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "PBGeneratedMessage.h"

@class NSString;

@interface WXCPbCSSubscribeLargeVideoReq : PBGeneratedMessage
{
    unsigned int hasRoomId:1;
    unsigned int hasRoomKey:1;
    unsigned int hasOwnerUserName:1;
    unsigned int hasGroupId:1;
    unsigned int hasTimestamp:1;
    int roomId;
    long long roomKey;
    NSString *ownerUserName;
    NSString *groupId;
    unsigned long long timestamp;
}

+ (id)parseFromData:(id)arg1;
- (void).cxx_destruct;
@property(nonatomic, setter=SetTimestamp:) unsigned long long timestamp; // @synthesize timestamp;
@property(readonly, nonatomic) BOOL hasTimestamp; // @synthesize hasTimestamp;
@property(retain, nonatomic, setter=SetGroupId:) NSString *groupId; // @synthesize groupId;
@property(readonly, nonatomic) BOOL hasGroupId; // @synthesize hasGroupId;
@property(retain, nonatomic, setter=SetOwnerUserName:) NSString *ownerUserName; // @synthesize ownerUserName;
@property(readonly, nonatomic) BOOL hasOwnerUserName; // @synthesize hasOwnerUserName;
@property(nonatomic, setter=SetRoomKey:) long long roomKey; // @synthesize roomKey;
@property(readonly, nonatomic) BOOL hasRoomKey; // @synthesize hasRoomKey;
@property(nonatomic, setter=SetRoomId:) int roomId; // @synthesize roomId;
@property(readonly, nonatomic) BOOL hasRoomId; // @synthesize hasRoomId;
- (id)mergeFromCodedInputStream:(id)arg1;
- (int)serializedSize;
- (void)writeToCodedOutputStream:(id)arg1;
- (BOOL)isInitialized;
- (id)init;

@end

